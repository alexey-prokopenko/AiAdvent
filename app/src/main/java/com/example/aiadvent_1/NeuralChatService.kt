...
class NeuralChatService {
    ...
    private val api = retrofit.create(NeuralChatApi::class.java)
    ...
    suspend fun generateResponse(userMessage: String): String {
        return withContext(Dispatchers.IO) {
            ...
            try {
                ...
                Log.d("NeuralChatService", "Время ответа модели: ${responseTime}ms (${String.format("%.2f", responseTimeSeconds)}s)")
                Log.d("NeuralChatService", "Токены - Входные: $promptTokens, Выходные: $completionTokens, Всего: $totalTokens")
                ...
                Log.e("NeuralChatService", "Ошибка HTTP ${e.code()} за ${responseTime}ms: $errorMessage")
                "Произошла ошибка HTTP ${e.code()}: $errorMessage"
            } catch (e: Exception) {
                val endTime = System.currentTimeMillis()
                val responseTime = endTime - startTime
                Log.e("NeuralChatService", "Ошибка за ${responseTime}ms: ${e.message}")
