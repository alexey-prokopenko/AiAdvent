    ...
    suspend fun generateResponse(userMessage: String): String {
        return withContext(Dispatchers.IO) {
            ...
            try {
                ...
                Log.d("HuggingFaceService", "Время ответа модели: ${responseTime}ms (${String.format("%.2f", responseTimeSeconds)}s)")
                Log.d("HuggingFaceService", "Токены - Входные: $promptTokens, Выходные: $completionTokens, Всего: $totalTokens")
                ...
                Log.e("HuggingFaceService", "Ошибка HTTP ${e.code()} за ${responseTime}ms: $errorMessage")
                "Произошла ошибка HTTP ${e.code()}: $errorMessage"
            } catch (e: Exception) {
                val endTime = System.currentTimeMillis()
                val responseTime = endTime - startTime
                Log.e("HuggingFaceService", "Ошибка за ${responseTime}ms: ${e.message}")
