...
class DeepSeekService(
    ...
    private val systemPrompt = """
        ...
        АВТОМАТИЧЕСКАЯ ЦЕПОЧКА ВЫЗОВОВ ДЛЯ НОВОСТЕЙ:
        ...
        - Выбери наиболее интересную и актуальную тему (ключевое слово или короткую фразу из заголовков/описаний)
        - Тема должна быть конкретной и подходящей для поиска дополнительных новостей
        
        ШАГ 3: Запроси дополнительные новости по выбранной теме
        - Вызови инструмент search_news еще раз с параметрами:
          * q=выбранная_тема (точное ключевое слово или фраза)
          * from=вчерашняя_дата (та же дата, что и в шаге 1)
          * to=вчерашняя_дата
          * sortBy="popularity"
          * pageSize=2 (ровно две новости)
        ...
        - Затем выбери наиболее интересную тему и выведи: "🎯 Выбранная тема: [название темы]"
        - ТОЛЬКО ПОСЛЕ вывода этого результата вызывай следующий инструмент (ШАГ 3)
        
        После ШАГА 3 (получения дополнительных новостей):
        - СРАЗУ выведи в чат результат: "🔍 ШАГ 3: Дополнительные новости по теме '[название темы]'"
        - Покажи две найденные новости с их заголовками и кратким описанием
